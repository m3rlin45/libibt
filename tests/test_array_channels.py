"""Tests for array variable (count > 1) channels.

Time-subsample arrays (`_ST` suffix) are merged into a single higher-rate
channel; other array variables are expanded into per-element channels
named `Name[i]`.
"""
...
def test_st_arrays_become_merged_channels(log, st_vars, raw_data):
    ...
    # non-_ST arrays (none in this file) would become per-element channels
    for name, _var_type, _offset, count in read_var_headers(raw_data):
        if count > 1 and not name.endswith("_ST"):
            assert f"{name}[0]" in log.channels
