...
- **Array variables** (count > 1): `_ST` time-subsample arrays are merged into one higher-rate channel under the variable's name; other arrays are expanded into one channel per element, named `Name[i]`
